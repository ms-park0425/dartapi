...
> **2603·2606**: 정답 데이터 없어 정확도 미검증. 추출 완료: 2603 73개 컬럼·742개 값, 2606 69개 컬럼·651개 값 (12개 회사).
...
- **2603 (미검증)**: 2026년 1분기. 73개 컬럼·742개 값 추출 (12개 회사). 정답 데이터 없어 정확도 미측정.
- **2606 (미검증)**: 2026년 반기. 69개 컬럼·651개 값 추출 (12개 회사). 2026년 신규 XBRL 구조(eHYA·eHYQ 분리) 대응. 정답 데이터 없어 정확도 미측정.
...
> **2026년 XBRL 구조 변경**: 반기·분기 보고서에서 BS context가 `eHYA`(반기누계)와 `eHYQ`(분기단독)로 분리. B/S 항목(총자산 등)은 `eHYA` 우선 사용. 2025년 이전은 `eHYA`만 존재해 자동 처리됨.
