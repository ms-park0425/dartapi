...
| 기간 | 보고서 종류 | DART만 OK율 | **IR 포함 OK율** | IR 포함 OK/FAIL/MISS | 비교대상 |
|------|-----------|----------:|----------------:|--------------------:|--------:|
| 2312 | 2023년 사업보고서 | 54% | — | — | 955 |
| 2412 | 2024년 사업보고서 | 71% | — | — | 1,096 |
| 2503 | 2025년 1분기 | 67% | **72%** | 672/31/227 | 930 |
| 2506 | 2025년 반기 | 70% | **73%** | 693/43/220 | 956 |
| 2509 | 2025년 3분기 | 80% | **82%** | 785/23/144 | 952 |
| 2512 | 2025년 사업보고서 | 88% | **91%** | 1,022/19/84 | 1,125 |
| 2603 | 2026년 1분기 | — | — | — | 미검증 |
| 2606 | 2026년 반기 | — | — | — | 미검증 |

> 2312·2412는 IR fill 검증 미실시 (정답 파일과 이미 불일치 0). 2503~2512는 `parse_annual.py` 후 `ir_fill.py` 적용 최종 수치 (2026-08-24 기준).
...
- **2506 (70%)**: 반기 보고서. 손보사 Col36~40(보험금융/재보험금융/금융손익/재산관리비/기타투자) 반기 기재 방식 차이 처리.
...
- **2603 (미검증)**: 2026년 1분기. 73개 컬럼·744개 값 추출 (12개 회사). 정답 데이터 없어 정확도 미측정.
- **2606 (미검증)**: 2026년 반기. 69개 컬럼·670개 값 추출 (12개 회사). 2026년 신규 XBRL 구조(eHYA·eHYQ 분리, 전기말CSM 3-dim) 대응. 정답 데이터 없어 정확도 미측정.
...
`parse_annual.py` 로 1차를 채운 뒤 `ir_fill.py` 를 돌린 전후를 정답과 대조했다.
오차 허용은 절대차 1 이하 또는 상대오차 0.1% 이내(기존 `verify_2512.py` 기준),
수식 CHECK 컬럼은 집계에서 제외했다.

| 기간 | | OK | FAIL | MISS | 합계 | OK율 |
|---|---|---:|---:|---:|---:|---:|
| 2503 | 전 | 624 | 28 | 278 | 930 | 67.1% |
|      | 후 | **672** | 31 | **227** | 930 | **72.3%** |
| 2506 | 전 | 667 | 41 | 248 | 956 | 69.8% |
|      | 후 | **693** | 43 | **220** | 956 | **72.5%** |
| 2509 | 전 | 760 | 22 | 170 | 952 | 79.8% |
|      | 후 | **785** | 23 | **144** | 952 | **82.5%** |
| 2512 | 전 | 992 | 19 | 114 | 1,125 | 88.2% |
|      | 후 | **1,022** | 19 | **84** | 1,125 | **90.8%** |
| **합계** | 전 | 3,043 | 110 | 810 | 3,963 | 76.8% |
|          | 후 | **3,172** | 116 | **675** | 3,963 | **80.0%** |

**MISS 810 → 675 (135셀 감소), OK +129.**

### 남은 MISS 675셀은 어디인가

| 성격 | 셀 | 상황 |
|---|---:|---|
| CSM 기간별(83~88) | 157 | **IR 자료에 항목 자체가 없다.** 사업보고서 주석이 정답 |
| 예실차(111~117) | 142 | 동양·메리츠 팩트시트에 없음(자동 탐색 확인). KB·DB·교보는 PDF |
| BEL·RA·CSM(73~81) | 106 | 대부분 미매핑 4개사 |
| 자본변동(55~71) | 75 | 팩트시트가 다루는 범위 밖 |
| 손해율(101~109) | 49 | 일부 회사만 팩트시트에 있음 |
| OCI 세부(24~30) | 40 | 미래에셋 SAP-BS 외에는 없음 |
| 가정변경(146~157) | 22 | IR 자료에 없음 |

회사별로 보면 **KB손보 67 · DB손보 53 · 교보 50 · 신한라이프 45 · KB라이프 43 = 258셀이
아직 매핑하지 않은 4개사**다. 여기가 남은 최대 덩어리이고 PDF 파싱으로 접근할 수 있다.
팩트시트를 내는 7개사는 사실상 짜낼 만큼 짜냈다.
...
| 한화생명 | Col21·Col44·Col48·Col108·Col74 — `(별도) 요약 손익/재무상태표`, `효율지표` | Col73(BEL) 16% 차이(발생사고요소 제외), Col109(사고보험금) 정의 불일치, Col22(운용자산) 특별계정 포함 여부 |
| 동양생명 | Col13·Col48·Col111·Col112·Col115 — `FS` 시트 | Col18·Col21·Col24·Col44·Col49·Col50 은 `FS` 가 **연결** 기준이라 DATA(별도)와 3% 안팎 차이 |
...
| 한화생명 | 56 | 0 | Col21·44·48·74·108 추가 |
| 삼성생명 | 46 | 2 | Col73·74 추가. 실패 2건은 2509 손해율(재작성) |
| 동양생명 | 45 | 2 | Col13·48·111·112·115 추가. 실패 2건은 2506 사업비예실차 재작성 |
| 삼성화재 | 42 | 0 | |
| DB손해보험 | **36** | **0** | 신규 |
| 메리츠화재 | 24 | 0 | |
| 현대해상 | 23 | 0 | |
